        ...
        <div className="mb-3 flex gap-2 text-xs">
          {(["All", "School (Science)", "Coding Bootcamp"] as const).map((category) => (
            <button
              ...
              className={`rounded-full px-3 py-1.5 font-semibold ${activeCategory === category
          ...
          <div className="flex gap-2 text-xs">
            {(["All", "School (Science)", "Coding Bootcamp"] as const).map((category) => (
              <button
                key={category}
                onClick={() => setActiveCategory(category)}
                className={`rounded-full px-4 py-2 font-bold transition border ${activeCategory === category
